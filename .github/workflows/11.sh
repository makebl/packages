  ...
  build:
    ...
    - name: Clone packages
      run: |
        ...
        git clone -b ${{matrix.target}} https://github.com/makebl/packages.git ${{matrix.target}}
        cd ${{matrix.target}}
    ...
    - name: Upload small-package
      ...
      run: |
        ...
        git push --quiet "https://${{ secrets.ACCESS_TOKEN }}@github.com/makebl/packages.git" HEAD:${{matrix.target}}
